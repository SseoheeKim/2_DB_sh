...
- 자원을 효율적으로 관리하기 위한 '다양한 정보'를 저장하는 시스템 테이블
...
-- DQL(DATA QUARY LANGUAGE) : 데이터 질의(조회)언어

-- DML(DATA MANIPULATION LANGUAGE) : 데이터 삽입, 수정, 삭제하는 언어
-- DDL(DATA DEFINITION LANGUAGE) : 테이블 등의 객체를 생성, 수정, 삭제하는 등 정의 언어
-- DCL(DATA CONTROL LANGUAGE) : 데이터 권한을 부여하거나 제한하는 언어

-- TCL(Transaction Control Language) : DML 수행 내용을 COMMIT, ROLLBACK하는 언어
...
-- 데이터의 전체 구조를 정의하는 언어로 주로 DB관리자(DBA), 설계자가 사용
 ...
 */
 ...
 /* 
    * 자료형 
 -  NUMBER : 숫자형 -> 정수, 실수의 구분없이 전부 
    
 -  CHAR(크기) : 고정길이 문자형 (2000BYTE) -> 비교적 속도 빠름
        -> ex) CHAR(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 10BYTE 저장공간을 모두 사용. 
        -> 주민번호나 전화번호와 같이 정해진 글자수에 사용
        
 -  VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE) -> 비교적 속도 느림
        -> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함.
        -> 이름, 주소, 게시글 등 전부 다른 글자수에 사용
        
        
 -  DATE : 날짜 타입(년/월/일/시/분/초)
    
 -  BLOB : 대용량 이진 데이터 (4GB) 
           이진수의 형태의 동영상/이미지/실행파일 등 저장. 단, 속도때문에 지양!
 
 -  CLOB : 대용량 문자 데이터 (4GB) -> 문자만 4GB 저장


 ----------------------------------------------------------------------------------
 - SQL작성법 : 대문자 작성 권장, 연결단어는 "_"(언더바)사용
 - 문자인코딩 UTF-8에 따라서 영어/숫자는 1BYTE,  한글/특수문자 등은 3BYTE취급
 
*/


-- 1.CREATE
-- 테이블이나 인덱스, 뷰 등 다양한 데이터베이스 '객체를 생성'하는 구문
...
-- USER_TABLES : 사용자가 작성한 테이블을 확인하는 (데이터 딕셔너리에 정의된) 뷰
SELECT * FROM USER_TABLES;


-- MEMBER 테이블에 샘플 데이터 삽입
INSERT INTO MEMBER VALUES ('mem01', '1234abc', '홍길동', '890411-1234567', DEFAULT);
--> INSERT/UPDATE에서 컬럼 값으로 DEFAULT를 작성하면 해당 컬럼에 지정된 DEFAULT 값으로 삽입


--  데이터  삽입 확인
SELECT * FROM "MEMBER";
COMMIT; --> COMMIT 전에는 DB에서 확인 불가


-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES ('mem02', 'abc1234', '박철수', '901118-1234567', SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES ('mem03', 'efg1234', '김영희', '920418-1234567', DEFAULT);


-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO "MEMBER"(MEMBER_ID, MEMBER_PWD, MEMBER_NAME) 
VALUES ('mem04', '1!324', '이지연');
--> DEFAULT값이 없으면 DB내에 NULL값이 삽입됨


-- ** JDBC에서 날짜를 입력받았을 때 삽입하는 방법 **
-- 2022-09-13 17:33:27
INSERT INTO "MEMBER" VALUES ('mem05', 'pass05', '김기동', '940303-2451477', 
...
-- MEMBER2테이블 생성(아이디, 비밀번호, 이름, 전화번호)
...
/* ** 제약조건명 지정 방법 **
 1) 컬럼 레벨 : 테이블 생성 시 컬럼을 정의하는 부분에 작성
				[CONSTRAINT 제약조건명] 제약조건

 2) 테이블 레벨 : 테이블 생성 시 컬럼 정의가 끝난 후, 마지막에 작성!!
...
-- 대부분의 경우는 NULL을 허용하지 X


CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL, --> '컬럼 레벨'에서 모든 사용자는 '사용자 번호가 존재'하여야 하는 제약조건 설정
    USER_ID VARCHAR2(20) ,
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
 -- USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨에서 제한 (제약조건명 미지정)
 -- USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- 컬럼 레벨에서 제한 (제약조건명 지정)
    USER_ID VARCHAR2(20),
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    ...
    EMAIL VARCHAR2(50),
    /*테이블 레벨*/ 
 -- UNIQUE(USER_ID) -- 테이블 레벨에서 제한(제약조건명 미지정)
 	CONSTRAINT USER_ID_U UNIQUE(USER_ID) -- 테이블 레벨에서 제한(제약조건명 지정)
);
...
--  ORA-00001: 무결성 제약 조건(KH_KSH.USER_ID_U)에 위배됩니다
...
-- 복합키는 지정된 모든 컬럼의 값이 같을 때만 위배된다

-- USER_ID, USER_NAME으로 복합키 지정 
CREATE TABLE USER_USED_UK2(
    USER_NO NUMBER,
    USER_ID VARCHAR2(20),
    USER_PWD VARCHAR2(30) CONSTRAINT USER_PWD_U NOT NULL,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    -- 테이블 레벨에서 UNIQUE복합키 지정
...
-- 가장 중요한 1순위 키, 필수 제약 조건
-- 테이블에서 한 행의 정보를 찾기 위해 사용할 컬럼을 의미
-- 테이블에 대한 식별자(IDENTIFIER) 역할 (임의로 지정된 주식별자)

-- NOT NULL + UNIQUE  --> 절대로 중복되지 않는 값이 필수 존재

-- 한 테이블당 한 개만 설정 가능 
-- 컬럼레벨 / 테이블레벨 둘다 설정 가능함
-- 한 개 컬럼에 설정할 수도 있고, 여러 개의 컬럼을 묶어서 설정(복합키 설정)할 수 있음


CREATE TABLE USER_USED_PK(
    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼레벨에서 제약조건명 지정하여 제한
...
--> 기본키 중복으로 오류
--  00001: 무결성 제약 조건(KH_KSH.USER_NO_PK)에 위배됩니다
...
-- FOREIGN KEY제약조건에 의해서 테이블간의 관계(RELATIONSHIP)가 형성 -> RDBMS
-- 제공되는 값 외에는 NULL만 사용 가능 -> NULL은 참조하는 컬럼값 X
...
-- * 참조될 수 있는 컬럼은 PRIMARY KEY컬럼과, UNIQUE 지정된 컬럼만 외래키로 사용할 수 있고,
--   참조할 테이블의 참조할 컬럼명이 생략되면 PRIMARY KEY로 설정된 컬럼이 자동으로 참조할 컬럼
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE/*(GRADE_CODE)*/ -- 컬럼 레벨에서 지정 
  											--> 컬럼명 미작성 시 USER_GRADE테이블의 PK를 자동 참조
  -- , CONSTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE
  --> FOREIGN KEY는 테이블레벨에서 제한할 때 사용하는 키워드
);
...
-- ORA-02291: 무결성 제약조건(KH_KSH.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
-- * FOREIGN KEY 삭제 옵션 
-- 부모 테이블의 데이터 삭제 시 자식 테이블의 데이터를 어떻게 처리할 지에 대한 내용 설정 가능
...
CREATE TABLE USER_USED_FK2(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK2 REFERENCES USER_GRADE2 ON DELETE SET NULL -- 컬럼레한 제한
  															          /* 삭제 옵션 */
);
...
--> ON DELETE SET NULL 옵션이 설정되어 있어 오류 없이 삭제되어 컬럼값이 NULL로 변경
...
-- 5. CHECK 제약조건 : 컬럼에 기록되는 값에 지정된 조건 설정을 할 수 있음
-- CHECK (컬럼명 비교연산자 비교값)
...
--> 둘 다 컬럼의 값을 지정된 형태로만 제한하기에 CHECK작성 후 수정이 번거롭지만,
---- FK는 부모테이블에 행을 추가, 수정, 삭제하는게 편리
...
CREATE TABLE USER_TEST( 
	...
	PNO VARCHAR(3) CONSTRAINT UK_PNO UNIQUE CONSTRAINT NN_PNO NOT NULL,
...
-- 1) 테이블 전체 복사
-- CREATE TABLE 복사테이블명 AS 서브쿼리

CREATE TABLE EMPLOYEE_COPY AS SELECT * FROM EMPLOYEE;
--> 서브쿼리의 조회 결과(RESULT SET) 모양으로 테이블 생성


SELECT * FROM EMPLOYEE_COPY;

-- 2) JOIN하여 원하는 컬럼만 테이블로 복사
...
--> 서브쿼리를 통해 테이블 생성 시
-- 테이블 형태(컬럼명, 데이터 타입, 값)와 제약조건 NOT NULL만 복사될 뿐
-- 제약조건, 코멘트는 복사되지 않기 때문에 별도 추가작업이 필요!!
