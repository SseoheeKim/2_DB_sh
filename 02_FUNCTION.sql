...
-- SELECT문의 SELECT절, WHERE절, ORDER BY절, GROUP BY절, HAVING절 사용가능

-- 단일 행 함수 : N개의 값을 읽고 N개의 결과를 반환
...
--> 문자열을 앞에서부터 검색해서 첫번째 B위치를 조회

SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL;
--> 문자열을 5번째부터 검사해서 첫번째 B위치 조회

SELECT INSTR('AABAACAABBAA', 'B', 5, 2) FROM DUAL;
--> 문자열을 5번째부터 검사해서 두번째 B위치 조회

SELECT EMP_NAME, EMAIL, INSTR(EMAIL, '@') FROM EMPLOYEE; 
--> EMPLOYEE테이블에서 사원명, 이메일, 이메일 중 '@(at)'위치 조회



-- SUBSTR : 컬럼이나 문자열에서 지정한 위치부터 지정된 길이만큼 문자열을 잘라내서 반환
...
SELECT TRIM(BOTH '#' FROM '#####안녕#####') FROM DUAL; --> 양쪽의 문자 제거
...
SELECT '절대값 같음'
FROM DUAL
WHERE 10 = ABS(-10); -- WHERE절에도 함수 작성 가능
...
SELECT ROUND(123.456) FROM DUAL; -- 123 , 소수점 위치 생략시 첫 번째 자리에서 반올림

SELECT ROUND(123.456, 1) FROM DUAL; /* 123.5 , 두 번째 자리에서 반올림
									 -> 두 번째 자리에서 반올림하여 소수점 1개 표시 */

SELECT ROUND(123.456, 0) FROM DUAL; -- 소수점 첫 번째 자리에서 반올림 (소수점 위치 기본 값 : 0)

SELECT ROUND(123.456, -1) FROM DUAL; /*소수점 0 번째 자리에서 반올림해서 소수점 -1자리 표현
									   -> 1의 자리에서 반올림해서 10자리부터 표현 */


/* CEIL(숫자 | 컬럼명) : 올림
   FLOOR(숫자 | 컬럼명) : 내림
   --> '소수점 첫째 자리'에서 올림/내림 처리
      (반올림불가, 위치 지정 불가)   */
...
/* SYSTIMESTAMP : SYSDATE + MS 단위 추가 
 	--> 'TIMESTAMP'는 특정 시간을 나타내거나 기록하기 위한 문자열 */
...
-- EMPLOYEE테이블에서 사원의 이름, 입사일, 근무한 개월 수, 근무 연차 조회
...
SELECT SYSDATE + 120 FROM DUAL; --> 부정확하기 때문에 ADD_MONTHS사용
...
/* 문자열로 변환 
 ...
 <숫자 변환 시 포맷 패턴>
  ...
  L : 현재 내가 사용하는 DB에 설정된 나라의 화폐 기호  
...
SELECT TO_CHAR(EXTRACT(MONTH FROM HIRE_DATE), '00') || '월' FROM EMPLOYEE; -- '02월'  
...
SELECT SYSDATE FROM DUAL; -- 2022-09-01 11:22:14.000

-- 2022/09/01 11:22:14 목요일
SELECT TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS DAY') FROM DUAL; 

-- 2022년 09월 01 (목)
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" (DY)') FROM DUAL; 
/* 년, 월, 일은 한글이기 때문에 날짜를 나타내는 패턴으로 인식이 안돼 오류 발생
...
--> 패턴을 적용해 작성된 문자열의 각 문자가 어떤 날짜의 형식인지 인지시킴

-- EMPLOYEE 테이블에서 각 직원이 태어난 생년월일(1991년 10월 23일)처럼 조회
SELECT EMP_NAME, 
...
-- R 패턴 : 1세기를 기준으로 절반(50년) 이상인 경우 이전 세기인 1900년대
--							 절반(50년)	미만인 경우	현재 세기인 2000년대		
...
/* NULL 처리 함수 
  ...
  1) NVL(컬럼명, 컬럼값이 NULL일때 바꿀 값) : NULL인 컬럼값을 다른 값으로 변경
  
  2) NVL2(컬럼명, 바꿀값1, 바꿀값2)
...
-- NVL2 --> EMPLOYEE 테이블에서 보너스를 받으면 'O', 안받으면 'X'
...
-- EX) 직원의 성별 구하기 (남: 1 / 여: 2)
SELECT EMP_NAME 직원명, DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남성', '2', '여성' ) 성별
FROM EMPLOYEE;
...
/* AVG(숫자가 기록된 컬럼명)  : 평균*/
-- EX) 전 직원 급여 평균
SELECT ROUND(AVG(SALARY)) "전 직원 급여 평균" FROM EMPLOYEE;

-- EX) 부서코드가 D9인 사원들의 급여 합, 평균
SELECT SUM(SALARY) "D9합계", ROUND(AVG(SALARY)) "D9평균"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
...
/*
   MIN(컬럼명) : 최소값 
   MAX(컬럼명) : 최대값
   --> 타입 제한 없음 (숫자 : 대/소, 날짜 : 미래(대)/과거(소), 문자열 : 문자 순서)
 */

-- EX) 급여 최소값, 가장 빠른 입사일, 이메일 알파벳 순서가 가장 빠른 이메일
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- EX) 급여 최대값, 가장 늦은 입사일, 이메일 알파벳 순서가 가장 느린 이메일
SELECT MAX(SALARY), MAX(HIRE_DATE), MAX(EMAIL)
FROM EMPLOYEE;

-- EX) EMPLOYEE 테이블에서 급여를 가장 많이 받는 사원의 이름, 급여, 직급 코드 조회
...
/* COUNT( * | 컬럼명) : 행 개수를 헤아려서 리턴
 ...
 - COUNT(*) : 'NULL을 포함'한 전체 행 개수를 리턴
 - COUNT(컬럼명) : 'NULL을 제외'한 실제 값이 기록된 행 개수를 리턴함
 
 - COUNT([DISTINCT] 컬럼명) : 중복을 제거한 행 개수를 헤아려서 리턴
 */


SELECT COUNT(*) FROM EMPLOYEE; -- 23명
--> 테이블 전체 행의 개수
...
SELECT DISTINCT DEPT_CODE FROM EMPLOYEE; -- 7명
--> 부서코드의 중복을 제거
...
-- EX) EMPLOYEE 테이블에서 성별이 남성인 사원의 수 조회
SELECT COUNT(*) "남성 수"
FROM EMPLOYEE
WHERE SUBSTR(EMP_NO, 8, 1) = '1'; 

SELECT SUM(DECODE(SUBSTR(EMP_NO, 8, 1), '1', 1, 0)) "남성 수"
FROM EMPLOYEE;