...
SELECT STUDENT_NO, TO_CHAR(POINT, 'FM9.00') 학점  -- TO_CHAR(NUMBER, 'FM9.00')
FROM TB_GRADE 
...
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
JOIN TB_PROFESSOR USING(PROFESSOR_NO)
WHERE CATEGORY = '인문사회'; -- JOIN 순서 변경
...
WHERE CATEGORY = '인문사회'; -- 별칭을 통해 JOIN
...
SELECT STUDENT_NO 학번, STUDENT_NAME 이름, DEPARTMENT_NAME 학과명, AVG(POINT) 평점
FROM TB_STUDENT 
JOIN TB_GRADE USING(STUDENT_NO)
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
WHERE ABSENCE_YN = 'N'
...
-- 18 - 풀이2) JOIN을 덜해서 비교적 속도가 빠른 방법 
SELECT STUDENT_NO, STUDENT_NAME 
FROM (
	SELECT STUDENT_NO, STUDENT_NAME, AVG(POINT)
	FROM TB_GRADE
...
-- ** 19. 춘 기술대학교의 "환경조경학과"가 속한 같은 계열 학과들의 학과별 전공과목 평점
