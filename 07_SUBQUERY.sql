...
/*  서브쿼리 유형
    ...
    - 단일행 (단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개일 때 (1행 1열)
...
ORDER BY JOB_CODE DESC;
--> SELECT절에 없는 컬럼(JOB_CODE)이지만
--- FROM - JOIN 내에 존재하는 컬럼이면 ORDER BY절에 사용가능
...
-- 선택함수 사용 
-- 1) DECODE(컬럼명, 값1, 1인 경우, 값2, 2인 경우, ..... , 일치하지 않는경우)
-- 2) CASE WHEN 조건1 THEN 값2, 
-- 		   WHEN 조건2 THEN 조건2
--		   ELSE 값
--	  END 별칭				
...
-- 다중열 서브쿼리
-- WHERE절에 작성된 컬럼 순서에 맞게 서브쿼리 컬럼과 비교하여 일치하는 행만 조회
			
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE 
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE -- 컬럼 순서 중요
								FROM EMPLOYEE 
								WHERE ENT_YN = 'Y' 
								AND SUBSTR(EMP_NO, 8, 1) = 2)
AND ENT_YN != 'Y';
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE 	
           ...
           -- 2000년도 입사 ->  EXTRACT(YEAR FROM HIRE_DATE) = '2000'
...
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '소속없음') DEPT_TITLE, JOB_NAME, HIRE_DATE 		
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
WHERE ENT_YN = 'N'
...
-- 1) 메인쿼리 1행의 DEPT_CODE를 서브쿼리에 대입
-- 2) SUB 실행
-- 3) 서브 결과를 이용해서 MAIN조건절 수행
...
--    SQL에서 '단일 값'을 가리켜 '스칼라'
--	  SELECT절에 작성되는 단일행 서브쿼리

-- 모든 직원의 이름, 직급, 전체사원 중 가장 높은 급여와의 차
SELECT EMP_NAME, JOB_CODE, SALARY, 
...
SELECT ROWNUM, EMP_NAME 이름, SALARY 급여
FROM EMPLOYEE
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;
--> SELECT해석 순서 때문에 급여 상위가 아닌, 조회순서 상위 5명의 순서 조회

--> 인라인뷰로 해결
-- 1) 이름, 급여를 급여 내림차순으로 조회한 결과를 인라인 뷰로 사용
-- 2) 메인 쿼리 조회시 ROWNUM 순서까지 조회
...
-- 3) DENSE_RANK() OVER : 동일한 순위 이후의 등수를 이후의 순위로 계산
